...
TEN. は、デイリー・ランキング・フレンド機能などのオンライン機能を利用した時に、匿名のプレイヤーIDを発行して名前、スコア、フレンド情報などを保存します。初回起動時にオンラインアカウントを自動作成することはありません。Google Play のアカウント削除要件の対象となる可能性が高いため、安全側ではアプリ内アカウントとして扱います。
...
プレイヤー削除API、アプリ内削除UI、外部削除ページを実装済みです。アプリのマイページから外部削除ページを発行でき、リンクは30日間有効な削除コードを含みます。削除コードを発行できない場合は、外部削除ページから問い合わせ先へ削除を申請できます。申請を受けた運営者は、本人確認後に管理手順に従って削除を実施します。
...
- データ削除手段: **Yes**（アプリ内削除 + `https://ten-api-production.kajima37.workers.dev/account-deletion` の削除コード操作または問い合わせによる申請）
...
- `0.1.5` が内部テストに公開済み
- `0.1.5` がクローズドテスト（Alpha）にアップロード済みで、Play Consoleの審査中
