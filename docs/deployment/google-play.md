...
ポリシーはPlay Consoleだけでなく、アプリ内からもアクセスできるようにします。現時点ではリポジトリに正式なプライバシーポリシーのページとアプリ内リンクがありません。法的な開発者名、問い合わせ先、保存期間、削除方針を確定したうえで別途実装が必要です。
...
現行コードにはプレイヤー削除API、アプリ内削除UI、外部削除フォームがありません。これは一般公開前のブロッカーです。
