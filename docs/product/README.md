...
- Service Worker による Web 版のオフラインキャッシュ、PWA マニフェスト、対応ブラウザでのインストール促進 UI
...
- Cloudflare Workers Static Assets 向け Web ビルドと、CapacitorによるAndroid・iOSプロジェクト
- Google AdMob によるリワード動画（+10秒の報酬）と低頻度のインタースティシャル広告、Web 用モック UI
- 運用向け管理画面（OAuth ログイン、プレイヤー検索、利用停止・解除、IP 単位の停止（期限付き・将来の登録も拒否）、スコアの論理削除（ランキングから除外）、監査ログ）
