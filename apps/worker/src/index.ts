...
function deletionPage(env: Env, code: string): Response {
  ...
  const email = htmlEscape(env.LEGAL_CONTACT_EMAIL ?? 'sn178178178@gmail.com')
  const mailto = `mailto:${email}?subject=${encodeURIComponent('TEN. アカウント削除の申請')}&body=${encodeURIComponent('TEN. アカウント削除を申請します。\n\n表示名またはプレイヤーID（分かる場合）：\n')}`
  return legalLayout(
    'アカウント削除',
    `<h1>アカウント削除</h1><p>プレイヤー、スコア、フレンド関連データは復元できません。</p>${code ? `<p>アプリから発行した削除コードを確認し、下のボタンを押してください。</p><form method="post" action="/api/account/delete"><input type="hidden" name="deletionCode" value="${escapedCode}"><button type="submit">アカウントを削除する</button></form>` : ''}<section><h2>アプリを利用できない場合</h2><p>アプリをアンインストールした、または削除コードを発行できない場合は、次のメールアドレスから削除を申請してください。分かる範囲で表示名やプレイヤーIDを記載してください。</p><p><a href="${htmlEscape(mailto)}">${email} に削除を申請する</a></p></section><p><a href="/privacy">プライバシーポリシー</a></p>`,
    env,
  )
}
...
function englishDeletionPage(env: Env, code: string): Response {
  const escapedCode = htmlEscape(code)
  const email = htmlEscape(env.LEGAL_CONTACT_EMAIL ?? 'sn178178178@gmail.com')
  const mailto = `mailto:${email}?subject=${encodeURIComponent('TEN. account deletion request')}&body=${encodeURIComponent('I request deletion of my TEN. account.\n\nDisplay name or player ID (if known):\n')}`
  return legalLayout(
    'Account deletion',
    `<h1>Account deletion</h1><p>Your player, scores, and social data cannot be restored.</p>${code ? `<p>Confirm the deletion code generated by the app and press the button below.</p><form method="post" action="/api/account/delete"><input type="hidden" name="deletionCode" value="${escapedCode}"><button type="submit">Delete account</button></form>` : ''}<section><h2>If you cannot use the app</h2><p>If you uninstalled the app or cannot generate a deletion code, request deletion by email. Include your display name or player ID if known.</p><p><a href="${htmlEscape(mailto)}">Request deletion by email: ${email}</a></p></section><p><a href="/privacy?lang=en">Privacy policy</a></p><p>Contact: <a href="mailto:${email}">${email}</a></p>`,
    env,
    'en',
  )
}
  ...
  app.post('/api/account/delete', async (c) => {
    ...
    return c.html(
      requestedLocale(c.req.raw) === 'en'
        ? '<!doctype html><html lang="en"><meta charset="utf-8"><title>Account deleted</title><p>Your account has been deleted. You can close this page.</p>'
        : '<!doctype html><html lang="ja"><meta charset="utf-8"><title>アカウント削除完了</title><p>アカウントを削除しました。このページを閉じてください。</p>',
    )
  })
